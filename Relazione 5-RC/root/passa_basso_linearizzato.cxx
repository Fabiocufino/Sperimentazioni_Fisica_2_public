...
void passa_basso_linearizzato()
{
    ...
    for (int i = 0; i < basso.freq.size(); i++)
    {
        if (basso.freq[i] <= 7.7 && basso.freq[i] >= 4)
        {
            freq_new.push_back(basso.freq[i]);
            trasf_new.push_back(basso.trasf[i]);
            sfas_new.push_back(basso.sfasamento[i]);
            err_trasf_new.push_back(basso.err_trasf[i]);
            err_sfas_new.push_back(basso.err_sfasamento[i] / M_PI_2);
        }
    }
    ...
    grafico_trasf->SetTitle("Passa basso");
    ...
    grafico_trasf->GetXaxis()->SetLimits(4., 7.7);
    ...
    grafico_sfas->GetXaxis()->SetLimits(4., 7.7);
    ...
    fit_on_json(f1, 2, grafico_trasf,freq_new, trasf_new, pb_trasf, "../Fit_results", "pb_trasf_linearizzato.json", 4, 7.7);
    fit_on_json(f2, 2, grafico_sfas, freq_new, sfasamento_normalizzato, pb_sfas, "../Fit_results", "pb_sfas_linearizzato.json", 4, 7.7);
    ...
    TGaxis *A1 = new TGaxis(7.7, 0.35, 7.7, 0.85, 0, 0.85, 510, "+U");
    ...
    TLine *line = new TLine(0, 0, 7.96, 0);
    ...
    run_test(scarti_sfas, freq_new, sfasamento_normalizzato, 0.05875270407285163, 0.13802484815418667);
    ...
    TLine *line2 = new TLine(0, 0, 7.96, 0);
