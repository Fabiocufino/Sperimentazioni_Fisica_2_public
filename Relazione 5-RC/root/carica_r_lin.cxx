...
void carica_r_lin()
{
    ...
    TVectorD root_ln_v_v0(carica.ln_v_v0.size(), carica.ln_v_v0.data());
    TVectorD root_err_tempo(carica.err_t.size(), carica.err_t.data());
    TVectorD root_err_ln_v_v0(carica.err_ln_v_v0.size(), carica.err_ln_v_v0.data());
    ...
    grafico->SetTitle("Carica R Fit Lineare");
    ...
    fit_on_json(f1, 2, grafico, carica.t, carica.ln_v_v0, carica_results, "../Fit_results", "carica_r_lineare.json");

    TPaveStats *ptstats;
    TText *ptstats_LaTex;
    fit_results fit_results;
    fit_results.root_from_json("../Fit_results", "carica_r_lineare.json");
    ...
    vector<double> err_t(carica.err_ln_v.size(), 0);
    TVectorD root_err_t(err_t.size(), err_t.data());

    vector<double> scarti;

    run_test(scarti, carica.t, carica.ln_v_v0, fit_results.b[0], fit_results.a[0]);
    ...
    grafico1->SetTitle("Residui Carica R Fit Lineare");
