...
double min_chi_variando_omega(double *x, double *par) //le x sono le delta e le y sono le omega, restituisce una z.
{
    ...
    double omega_min = 319800;
    double omega_max = 321000;//320900
    double omega_step = 100;

    double chi = 0;
    vector<double> chi_per_ogni_a;

    for (double omega = omega_min; omega < omega_max; omega = omega + omega_step)
    {
...
void profilingO()
{
    ...
    TF2 *chi2 = new TF2("Minimizzazione del chi quadro variando #Omega", min_chi_variando_omega, 0.93, 1.07, 8800, 9700, 1);
    chi2->GetXaxis()->SetTitle("a");
    chi2->GetYaxis()->SetTitle("#delta");
