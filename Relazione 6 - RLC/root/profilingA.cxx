...
double min_chi_variando_a(double *x, double *par) //le x sono le delta e le y sono le omega, restituisce una z.
{
    ...
    double a_min = 0.9899;
    double a_max = 1.0199;
    double a_step = 0.0001;

    double chi = 0;
    vector<double> chi_per_ogni_a;

    for (double a = a_min; a < a_max; a = a + a_step)
    {
...
void profilingA()
{
    ...
    TF2 *chi2 = new TF2("Minimizzazione del chi quadro variando A", min_chi_variando_a, 8700, 9300, 319033, 323000, 1);
    chi2->GetXaxis()->SetTitle("#delta");
